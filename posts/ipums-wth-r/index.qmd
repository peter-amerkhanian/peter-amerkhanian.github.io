...
-   **Households**: Non group quarter housing units, defined by
    ...
    of the `GQ` variable documentation explains this choice in greater
    detail, but in short it represents non-institutional housing-units
    where 9 or fewer people unrelated to the head-of-household reside.
...
`{r} format(sample_household_true, scientific=FALSE)` has `{r} n` members,
each with a unique `PERNUM`. To identify household members' relationship
to the head-of-household, [we reference
...
quarters unit would have a very high "household income" were it included
...
an "institutional inmate" (`RELATE==13`) with no income.

So, given that our motivating question concerns household income, we can
apply the following filtering and isolate just the heads of all the
...
example, here we'll finally address the motivating question. **The
