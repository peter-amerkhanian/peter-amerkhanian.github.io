...
The following are some notes on using integration for finding cumulative probability distribution functions in the univariate and joint/multivariate settings. The exercises and citations are all coming out of a textbook for a course in vector calculus [@strang_calculus_2016-1]. I wrote this out because:  

1. I always forget how to calculate integrals and wanted to make some notes on a basic application of [$u$ subsitition](https://en.wikipedia.org/wiki/Integration_by_substitution) and the mechanics of multiple integration over general regions
2. I think that it's useful to pair the long, step-by-step integration walkthroughs with equivalent, much easier solutions using a [computer algebra system](https://en.wikipedia.org/wiki/Computer_algebra), a.k.a. symbolic computation. In practice I use [`sympy`](https://www.sympy.org/en/index.html) for all symbolic computation.

3. I think this application area, modeling waiting times, is interesting and relevant for policy analysis
...
#### Solving with symbolic computation
...
Now I'll switch to python code and set up some preliminaries -- I load the library, define the variables, and define the constant $e$ just so I can match the notation in the problem set-up:
...
Integration is not trivially easy -- to get the indefinite integral, we can just call `.integrate()` and supply the variable of integration.
```{python filename="python"}
...
#### Solving using symbolic computation
