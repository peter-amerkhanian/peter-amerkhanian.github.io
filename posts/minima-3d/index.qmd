...
\end{align}
$$
Now, given that this function describes the distance between $P = (-3, -5, 0)$ and any other point in $\mathbb{R}^3$, it follows that, without constraints, the closest point would just be $P$:  
...
\end{align}
$$
(0 is the smallest possible real number output of a square root function).
...
of a constraint -- we optimize for the closest point possible to our
target, $P_1$, subject to the constraint that the point must lie on the
surface, $z^2 = x^2 + y^2$.\
$$
\begin{align}
